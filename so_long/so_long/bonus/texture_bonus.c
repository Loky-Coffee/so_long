...
/*   Updated: 2023/12/15 17:20:04 by aalatzas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_position_update(t_map *map, t_tx *tx)
{
	...
	if (map->mapdata[oldy][oldx] != 'E' || map->mapdata[oldy][oldx] == 'G')
...
void	my_key_hook(mlx_key_data_t key_data, void *param)
{
		...
		&& ist_valid_key(key_data))
	{
		...
		if (map->mapdata[map->t_s.new_chary][map->t_s.new_charx] == 'G')
			errormsg(ENEMY_FOUND_IN_MAP);
		if (map->mapdata[map->t_s.new_chary][map->t_s.new_charx] != '1' &&
			map->mapdata[map->t_s.new_chary][map->t_s.new_charx] != 'G')
		{
